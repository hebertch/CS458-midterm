...
var update_bar_chart = function(csv_text) {
    console.log(csv_text);
    var width = 1950;
    var parsed_points = Papa.parse(csv_text, {header: true}).data;
    var data_points = parsed_points.map(function(data) { return {label: data.id, value: data.value}; });
    var data_points = data_points.filter(function(data) { return data.value; });
	...
	setup_canvas_context();
       ...
       getData();
        ...
        $scope.updateVisual = function (data) {
            ...
            loadVisual(file);
	    d3.request(file).mimeType('text/csv').get(function (xhr) {
		var csv_text = xhr.responseText;
		CurrentCSVText = csv_text;
		update_bar_chart(csv_text);
	    });
            flag = true;
